...
為避免 `.apkg` 因圖片過大，程式會以 180 DPI、JPEG 品質 90 產生圖片。這在細節與檔案大小間取得平衡，也比無損 PNG 大幅節省容量；若要調整，可修改 `create_anki_deck.py` 開頭的 `IMAGE_DPI` 和 `IMAGE_JPEG_QUALITY`。
...
**在專案資料夾裡 (在 `rad-test-main`，而非 `input`)** 開啟終端機，然後輸入：
